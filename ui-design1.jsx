 ...
 */
function Fraction({ numerator, denominator, className, alignTo = "center" }) {
  // alignTo: "center" | "top" | "bottom" (for future flexibility)
 ...
 */
function MixedNumber({ whole, numerator, denominator, className }) {
  return (
    <span
      className={classNames(
    ...
    >
      <span
        className="mr-0.5 font-math text-lg flex-shrink-0 leading-tight"
        style={{ minWidth: "0.7em", textAlign: "right" }}
      >
        {whole}
      </span>
      ...
      {tokens.map((token, idx) => {
        ...
        if (cleanedToken === "___")
          return (
            <span
              key={idx}
              className="inline-block border-b border-dashed border-blue-300 w-16 align-middle mx-2"
        ...
        return (
          <span
            key={idx}
            className="mx-0.5 select-none font-math text-xl leading-tight"
            style={{ minWidth: "0.8em", textAlign: "center", display: "inline-block", verticalAlign: "middle" }}
          >
            {cleanedToken}
          </span>
        );
...
const MathsTutoringPage = () => {
  ...
  const subCategories = selectedCategory
    ? Object.keys(WORKSHEETS[selectedCategory])
    : [];

  // Get worksheets for selected subcategory
  const worksheets =
    selectedCategory && selectedSubCategory
      ? WORKSHEETS[selectedCategory][selectedSubCategory]
      : [];

  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-blue-100 flex flex-col items-center py-8 px-2 print:bg-white print:text-black">
      {/* Header */}
                ...
                <li key={cat} className="mb-2">
                  ...
                  {selectedCategory === cat && (
                    <ul className="ml-4 mt-1">
                      {Object.keys(WORKSHEETS[cat]).map((subCat) => (
                        <li key={subCat}>
                          <button
                            className={classNames(
          ...
          {selectedWorksheet ? (
                ...
                <ol className="space-y-4 text-lg text-blue-900 print:text-black">
                  {selectedWorksheet.questions.map((q, idx) => (
                    <li key={idx} className="mb-3">
                      ...
                      <div className="border-b border-dashed border-blue-300 mt-1 mb-2 print:border-black" style={{ minHeight: "1em" }} />
                    </li>
                  ))}
                </ol>
            ...
            <>
              ...
              {selectedCategory && selectedSubCategory ? (
                ...
                <div className="flex flex-col items-center justify-center min-h-[200px] text-blue-500">
...
export default MathsTutoringPage;