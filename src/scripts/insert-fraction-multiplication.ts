...
async function insertProblems() {
  try {
    console.log('Reading fraction multiplication problems from JSON file...\n');

    const jsonPath = path.join(__dirname, '../../temp/fraction-multiplication-problems.json');
    ...
    const problems = jsonProblems.map(p => {
      ...
      return {
        question: p.question,
        answer: p.answer,
        type: 'FRACTION_MULTIPLICATION' as const,
    ...
    // Verify
    const totalMultiplication = await prisma.problem.count({
      where: {
        type: 'FRACTION_MULTIPLICATION',
      },
    });
    ...
    console.log(`\nTotal FRACTION_MULTIPLICATION: ${totalMultiplication}`);
