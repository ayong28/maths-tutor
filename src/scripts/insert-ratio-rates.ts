...
async function main() {
  console.log('📊 Starting ratio & rates problems insertion...\n');

  // Read JSON file
  const jsonPath = join(__dirname, '../../temp/ratio-rates-problems.json');
  ...
  const simplifyRatio = problems.filter(p => p.tags.includes('simplify-ratio')).length;
  const divideQuantity = problems.filter(p => p.tags.includes('divide-quantity')).length;
  const rateProblems = problems.filter(p => p.tags.includes('rate')).length;
  const bestBuy = problems.filter(p => p.tags.includes('best-buy')).length;

  console.log('\n🎯 Problem types:');
  console.log(`  Simplify ratios: ${simplifyRatio}`);
  console.log(`  Divide quantities: ${divideQuantity}`);
  console.log(`  Rate problems: ${rateProblems}`);
  ...
  // Verify RATIO_RATES count
  const ratioRatesCount = await prisma.problem.count({
    where: { type: 'RATIO_RATES' },
  });
  console.log(`📊 RATIO_RATES problems: ${ratioRatesCount}`);
}
